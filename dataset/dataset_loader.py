...
class SNDataset(Dataset):           #处理土壤和遥感数据
  ...
  def __getitem__(self, index):
    ...
    point_id = l8_img_name.split('.')[0]
    print(f"File name: {l8_img_name}, Extracted Point ID: {point_id}")

    print("DataFrame Point_ID type:", self.df['Point_ID'].dtype)
    print("Sample Point_ID values:", self.df['Point_ID'].head())
    ...
    if row.empty:
        raise ValueError(f"No matching Point_ID found for {point_id}")
    oc = row['OC'].values[0]
...
class SNDatasetClimate(Dataset):#处理气候数据
  ...
  def __getitem__(self, index):
    ...
    # 找到有效数组的形状
    target_shape = max((df.values.shape for df in self.clim_dfs_row if df.values.size > 0), default=(0, 61))

    filled_clim_dfs = [
      df.values if df.values.size > 0 else np.zeros(target_shape) for df in self.clim_dfs_row
    ]
    #clim_arr = np.stack([df.values.squeeze() for df in self.clim_dfs_row], axis=1)
    clim_arr = np.stack([filled_clim_dfs for df in self.clim_dfs_row], axis=1)
    
    clim_new = torch.zeros([13,61])
    for i in range(clim_arr.shape[0]):
      for j in range(clim_arr.shape[3]):
        clim_new[i][j] = clim_arr[i][0][0][j]
    ...
    if self.transform:
        ...
        clim_new = clim_new.clone().detach().to(dtype=self.clim_dtype)
    clim_new  = clim_new.transpose(1,0)
    ...
    if self.return_point_id:
        ...
        return (l8_img,clim_new),oc
        # return (l8_img,clim_arr),oc
class SNDatasetCorr(Dataset):#处理光谱数据
  def __init__(self, l8_dir, csv_dir , climate_csv_folder,spectral_csv_path,
               l8_bands: list = None ,transform = None,
               dates = ['20100101', '20100201', '20100301', '20100401', '20100501', '20100601',
                        '20100701', '20100801', '20100901', '20101001', '20101101', '20101201',
                        '20110101', '20110201', '20110301', '20110401', '20110501', '20110601',
                        ...
                        '20140101', '20140201', '20140301', '20140401', '20140501', '20140601',
                        '20140701', '20140801', '20140901', '20141001', '20141101', '20141201', '20150101'],
               climate_dtype = torch.float32, normalize_climate = True,normalize_spectral=True, return_point_id = False):
    """_summary_
    ...
    if normalize_climate:
        ...
        self.clim_dfs = [norm_clim(clim_df) for clim_df in self.clim_dfs]
            # 新增：光谱数据处理
        self.spectral_df = pd.read_csv(spectral_csv_path)
        if normalize_spectral:
            self.scaler = StandardScaler()
            spectral_columns = [col for col in self.spectral_df.columns 
                              if col not in ['PointID', 'NUTS_0', 'OC']]
            self.spectral_df[spectral_columns] = self.scaler.fit_transform(
                self.spectral_df[spectral_columns])
    self.dates = dates
    self.clim_dtype = climate_dtype
  ...
  def __getitem__(self, index):
    ...
    if self.l8_bands: l8_img = l8_img[self.l8_bands,:,:]
    # 获取光谱数据
    spectral_row = self.spectral_df[self.spectral_df['PointID'] == int(point_id)]
    spectral_data = spectral_row.drop(['PointID', 'NUTS_0', 'OC'], axis=1).values
    spectral_tensor = torch.tensor(spectral_data, dtype=self.clim_dtype)
    ...
    clim_new  = clim_new.transpose(1,0)
        # 返回数据
    if self.return_point_id:
        return (l8_img, clim_new, spectral_tensor), oc, point_id
    else:
        return (l8_img, clim_new, spectral_tensor), oc
...
class myNormalize:#归一化处理，处理不同波段的数据范围，处理缺失值
...
class myToTensor:#将Numpy数组转换为PyTorch张量
...
class Augmentations:#数据增强，图像填充，随机翻转，随机旋转，中心裁剪
...
class TensorCenterPixels:#提取图像中心区域，支持双线性插值，处理不同大小的输入
...
class NormalizeClimDF:#处理时间序列气候数据，数据归一化，处理缺失值
    def __init__(self,
