...
#     parser.add_argument('-srtm', '--use_srtm', action='store_true', default=USE_SRTM, help='Use SRTM data')
#     parser.add_argument('-sa', '--use_spatial_attention', action='store_true', default=USE_SPATIAL_ATTENTION, help='Use spatial attention')
#     parser.add_argument('-cnn', '--cnn_architecture', type=str, default=CNN_ARCHITECTURE, choices=['vgg16', 'resnet101', 'ViT','ViM'], help='CNN architecture')
#     parser.add_argument('-rnn', '--rnn_architecture', type=str, default=RNN_ARCHITECTURE, choices=['LSTM', 'GRU', 'RNN', 'Transformer','mamba'], help='RNN architecture')
#     parser.add_argument('-rv', '--reg_version', type=int, default=REG_VERSION, choices=[1, 2], help='Regression version')
#     parser.add_argument('-lstm', '--use_lstm_branch', action='store_true', default=0, help='Use Climate data (I know! the name is misleading!)') 
#     parser.add_argument('-s', '--seeds', nargs='+', type=int, default=SEEDS, help='Seeds for cross validation')
...
#         train_dl = DataLoader(train_ds, batch_size=TRAIN_BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS,drop_last=True)
#         test_dl = DataLoader(test_ds, batch_size=TEST_BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS,drop_last=True)
#         val_dl = DataLoader(val_ds, batch_size=TEST_BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS,drop_last=True)
...
import os
from soilnet.soil_net import SoilNet, SoilNetSimCLR,SoilNetSimCLR_CORR
...
# CONFIG
EXP_NAME = 'LUCAS_Self560_MiV_Trans_lstm'
NUM_WORKERS = 2
TRAIN_BATCH_SIZE = 4
TEST_BATCH_SIZE = 4
LEARNING_RATE = 1e-4
...
if __name__ == '__main__':
    ...
    parser.add_argument('-srtm', '--use_srtm', action='store_true', default=USE_SRTM, help='Use SRTM data')
    parser.add_argument('-sa', '--use_spatial_attention', action='store_true', default=USE_SPATIAL_ATTENTION, help='Use spatial attention')
    parser.add_argument('-cnn', '--cnn_architecture', type=str, default=CNN_ARCHITECTURE, choices=['vgg16', 'resnet101', 'ViT','MiV'], help='CNN architecture')
    parser.add_argument('-rnn', '--rnn_architecture', type=str, default=RNN_ARCHITECTURE, choices=['LSTM', 'GRU', 'RNN', 'Transformer','pathformer','itransformer'], help='RNN architecture')
    parser.add_argument('-rv', '--reg_version', type=int, default=REG_VERSION, choices=[1, 2], help='Regression version')
    parser.add_argument('-lstm', '--use_lstm_branch', action='store_true', default=0, help='Use Climate data (I know! the name is misleading!)') 
    parser.add_argument('-s', '--seeds', nargs='+', type=int, default=SEEDS, help='Seeds for cross validation')
    parser.add_argument('-snt', '--spectral_net_type', type=str, default='vgg', choices=['vgg', 'resnet', 'densenet','lstmcnn','LSTM'], help='Spectral network architecture type')
    parser.add_argument('-sf', '--spectral_in_features', type=int, default=2000,help='Number of input features for spectral data')
    parser.add_argument('-sh', '--spectral_hidden', type=int, default=512,help='Hidden dimension for spectral network')
    ...
    ################################### IF USE_LSTM_BRANCH #################################
    elif USE_LSTM_BRANCH and not USE_SPECTRAL: # USING THE CLIMATE DATA
        train_ds = SNDatasetClimate(train_l8_folder_path,
                                        lucas_csv_path,
                                        climate_csv_folder_path,
                                        l8_bands=bands, transform=train_transform)
