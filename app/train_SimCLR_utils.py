...
class SimCLR(nn.Module):
    ...
    def forward(self, feats1, feats2, feats3):
        ...
        # Concatenate three batches of features
        feats = torch.cat([feats1, feats2, feats3], dim=0)  # Shape: [3N, D]
        
        # Calculate cosine similarity
        cos_sim = F.cosine_similarity(feats[:, None, :], feats[None, :, :], dim=-1)  # Shape: [3N, 3N]
        ...
        # Set positive pairs for each modality
        for i in range(3):
            for j in range(3):
                if i != j:
                    # Create positive pairs between different modalities
                    start_i = i * batch_size
                    start_j = j * batch_size
                    pos_mask[start_i:start_i+batch_size, 
                           start_j:start_j+batch_size] = torch.eye(batch_size, dtype=torch.bool)
        ...
        log_prob = cos_sim[pos_mask] - torch.log(exp_sim.sum(dim=1)).repeat_interleave(2)
        loss = -log_prob.mean()
        
        # Calculate accuracy metrics
        with torch.no_grad():
            # For each sample, get its positive pairs
            total_samples = cos_sim.shape[0]
            rankings = []
            
            for i in range(total_samples):
                pos_indices = pos_mask[i].nonzero().squeeze()
                if pos_indices.dim() == 0:
                    pos_indices = pos_indices.unsqueeze(0)
                
                # Get similarity scores for current sample
                sample_sim = cos_sim[i]
                
                # Get rankings of positive pairs
                _, sorted_indices = sample_sim.sort(descending=True)
                for pos_idx in pos_indices:
                    ranking = (sorted_indices == pos_idx).nonzero().item()
                    rankings.append(ranking)
            
            rankings = torch.tensor(rankings, device=cos_sim.device)
            acc_top1 = (rankings == 0).float().mean()
            acc_top5 = (rankings < 5).float().mean()
            acc_mean_pos = 1 + rankings.float().mean()
...
def train_step(model:nn.Module, data_loader:DataLoader, loss_fn:nn.Module, optimizer:torch.optim.Optimizer):
    ...
    for batch, (X, y) in enumerate(loop):
        ...
        if isinstance(X, tuple) or isinstance(X, list):
            # 修改这里来处理三种输入：图像、气候和光谱数据
            img_data, climate_data, spectral_data = X
            img_data = img_data.to(device)
            climate_data = climate_data.to(device)
            spectral_data = spectral_data.to(device)
            X = (img_data, climate_data, spectral_data)
            y = y.to(device)
        ...
        # 1. Forward pass - 假设模型已经更新以处理三个输入
        z_img, z_clim, z_spec = model(X)

        # 2. Calculate and accumulate loss - 需要修改损失函数以处理三个特征向量
        loss, acc_top1, acc_top5, acc_mean_pos = loss_fn(z_img, z_clim, z_spec)
        train_loss += loss.item()
        train_top1 += acc_top1.item()
        train_top5 += acc_top5.item()
        train_mean_pos += acc_mean_pos.item()
...
def test_step(model:nn.Module, data_loader:DataLoader, loss_fn:nn.Module, verbose = False):
    ...
    if verbose:
        print(f"Test Loss: {test_loss:>8f}%")
        print(f"Shapes - img: {z_img.shape}, clim: {z_clim.shape}, spec: {z_spec.shape}")
    return test_loss, test_top1, test_top5, test_mean_pos
