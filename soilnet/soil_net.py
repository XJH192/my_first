...
from submodules.spe import VGGSpectraNet,ResSpectraNet,DenseSpectraNet,LSTMCNNAttentionNet,LSTM,PLSRNet,SVM,CNNNet,LSSVM,CNNNetWithAttention
try:
    from submodules.spe import MambaCNNAttentionNet, RFSpectraNet
except ImportError:
    MambaCNNAttentionNet = None
    RFSpectraNet = None
...
class SoilNetCORR(nn.Module):
    ...
    # 选择光谱网络架构
        if spectral_net_type == "vgg":
            ...
            if MambaCNNAttentionNet is None:
                raise ImportError("MambaCNNAttentionNet 未在 submodules.spe 中定义")
            self.spectral_net = MambaCNNAttentionNet(spectral_in_features,hidden_size)
        elif spectral_net_type == "rf":
            if RFSpectraNet is None:
                raise ImportError("RFSpectraNet 未在 submodules.spe 中定义")
