...
# class TSMambaEncoderClassiregressor(nn.Module):
#     """Time Series Mamba Classifier/Regressor maintaining original IO interface."""
#     def __init__(
#         self,
#         feat_dim: int,
#         max_len: int,
#         d_model: int,
#         n_heads: int,        # Kept for compatibility (not used)
#         num_layers: int,
#         dim_feedforward: int,
#         num_classes: int,
#         dropout: float = 0.1,
#         pos_encoding: str = "fixed",  # Kept for compatibility (not used)
#         activation: str = "gelu",
#         norm: str = "BatchNorm",      # Kept for compatibility (not used)
#         freeze: bool = False,         # Kept for compatibility (not used)
#     ):
#         super().__init__()
#         self.max_len = max_len
#         self.d_model = d_model
...
#             MambaBlock(d_model, dim_feedforward, dropout, activation)
...
#         # Apply padding masks
#         inp = inp * padding_masks.unsqueeze(-1).to(inp.dtype)
...
#         output = output * padding_masks.unsqueeze(-1).to(output.dtype)
...
# if __name__ == "__main__":
#     print("Testing TSMambaEncoderClassiregressor...")
#     x = torch.rand(2, 3, 4)  # (batch_size, seq_length, feat_dim)
#     padding_masks = torch.tensor([[True, True, True], [True, True, True]])
    
#     model = TSMambaEncoderClassiregressor(
#         feat_dim=4,
#         max_len=3,
#         d_model=4,
#         n_heads=2,          # Not used in Mamba
#         num_layers=2,
#         dim_feedforward=8,  # Should be d_model * expand (4*2=8)
#         num_classes=1,
#         dropout=0.1,
#     )
