...
#         output = self.concat_fc(concatenated)
#         return output


# class MultiHeadRegressor(MultiHeadRegressorBase):
#     def __init__(self, *input_sizes:int, hidden_size=128, activation="sigmoid", version=1, 
#                  dropout_prob=0.5, spectral_weight=0.6, image_weight=0.2, climate_weight=0.2):
#         super().__init__(*input_sizes, hidden_size=hidden_size, activation=activation)
...
#         # 分别编码各个输入
#         # 假设inputs的顺序是: [image_features, climate_features, spectral_features]
#         encoded = [encoder(x) for encoder, x in zip(self.encoders, inputs)]
        
#         # 应用权重
#         # 确保权重和为1
#         total_weight = self.spectral_weight + self.image_weight + self.climate_weight
#         image_weight_norm = self.image_weight / total_weight
#         climate_weight_norm = self.climate_weight / total_weight
#         spectral_weight_norm = self.spectral_weight / total_weight
        
#         # 应用权重到对应特征
#         # encoded[0] - 图像特征, encoded[1] - 气候特征, encoded[2] - 光谱特征
#         weighted_encoded = [
#             encoded[0] * image_weight_norm,
#             encoded[1] * climate_weight_norm,
#             encoded[2] * spectral_weight_norm
#         ]
        
#         # 连接加权特征
#         concatenated = torch.cat(weighted_encoded, dim=1)
